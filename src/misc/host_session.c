...
static void run_login(int pty, char **argv, char **env)
{
	int i;
	...
	if (ioctl(pty, TIOCSCTTY, 0) < 0 && errno != EPERM)
		/* already controlling from the sandboxed child; continue */;
	...
	for (i = 3; i < 256; ++i)
		close(i);
	execve(argv[0], argv, env && env[0] ? env : environ);
	_exit(127);
}
